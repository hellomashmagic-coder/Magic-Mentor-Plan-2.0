...
export default function App() {
    ...
    fetch('https://api.ipify.org?format=json')
      ...
      .then(data => {
        ...
        if (savedStatus === 'true' && savedIp === currentIp) {
          setIsApproved(true);
        } else if (savedStatus === 'true' && savedIp !== currentIp) {
          console.warn('IP mismatch detected. Re-approval required.');
          setIsApproved(false);
      ...
      .catch(err => {
        ...
        // Fallback or strict lockout can be implemented here
      });
  }, []);
  ...
  const handleApprove = () => {
    if (pendingRequest) {
      ...
      localStorage.setItem('magic_mentor_ip', pendingRequest.ip);
  ...
  if (!isApproved) {
    return (
      <>
        <AccessGate onRequested={handleRequestAccess} />
        <ApprovalSimulator 
